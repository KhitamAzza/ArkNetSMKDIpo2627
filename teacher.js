...
async function loadTeacherDashboard() {
  ...
  const cacheKey = "teacher_dash_" + currentGuru.kelas;
  const cachedRaw = sessionStorage.getItem(cacheKey);

  if (cachedRaw) {
    try {
      const cached = JSON.parse(cachedRaw);
      renderTeacherDashboard(cached);
    } catch (e) {
  ...
  try {
    const res = await fetch(API_URL + "?action=getTeacherDashboard&kelas=" + encodeURIComponent(currentGuru.kelas));
    ...
    console.error(err);
    if (!cachedRaw) showStudentToast("Error koneksi", "error");
  }
...
function renderTeacherDashboard(data) {
  ...
  renderValidationReady(data.validationReady || []);
}
...
async function showDataEkskulKelas() {
  ...
  const cacheKey = "teacher_class_" + currentGuru.kelas;
  const cachedRaw = sessionStorage.getItem(cacheKey);

  if (cachedRaw) {
    try {
      const cached = JSON.parse(cachedRaw);
      renderDataEkskulList(cached.data || []);
    } catch (e) {
      sessionStorage.removeItem(cacheKey);
    }
  }
  ...
  showStudentLoading(true);
  try {
    const res = await fetch(API_URL + "?action=getTeacherClassFull&kelas=" + encodeURIComponent(currentGuru.kelas));
    const data = await res.json();
...
function renderGuruAlertDetail(data) {
  ...
  const a = data.alerts || {};
  const hasAny = a.tidakMemilikiEkskul?.length || a.alphaTinggi?.length || a.pendingLama?.length || a.adaDenda?.length || a.belumSyarat?.length;
  ...
  if (a.adaDenda?.length > 0) {
    const g = document.createElement("div");
    g.className = "alert-group";
    g.innerHTML = `<div class="alert-group-title" style="color:var(--red);">Ada Denda (${a.adaDenda.length})</div>`;
    a.adaDenda.forEach(s => {
      g.innerHTML += `
        <div class="alert-item">
          <div class="alert-item-dot red"></div>
          <div class="alert-item-name">${s.nama}</div>
          <div class="alert-item-meta">Rp ${s.sisa.toLocaleString("id-ID")}</div>
        </div>`;
    });
    detail.appendChild(g);
  }

  if (a.belumSyarat?.length > 0) {
    const g = document.createElement("div");
    g.className = "alert-group";
    g.innerHTML = `<div class="alert-group-title" style="color:var(--yellow);">Belum Syarat Khusus (${a.belumSyarat.length})</div>`;
    a.belumSyarat.forEach(s => {
      g.innerHTML += `
        <div class="alert-item">
          <div class="alert-item-dot yellow"></div>
          <div class="alert-item-name">${s.nama}</div>
