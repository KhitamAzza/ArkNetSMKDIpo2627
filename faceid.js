...
// Reads face descriptors directly from Supabase Database table
// ============================================
...
// ===== VECTOR PARSER (handles array / JSON string / Postgres literal) =====
function parseFaceVector(val) {
  ...
  const arr = cleaned.split(',').map(v => parseFloat(v.trim())).filter(n => !isNaN(n));
...
async function initFaceId() {
  ...
  if (!faceIdModelsLoaded) {
    ...
    // 2. Load face descriptors from Supabase Database table
  showFaceIdLoading(true, "Memuat data wajah...");

  // === SAFE CACHE ATTEMPT ===
  try {
    const cacheKey = safeCacheKey('faceid_desc', currentSemester);
    const cached = await safeCacheGet(cacheKey);
    if (cached && typeof cached.ts === 'number' && (Date.now() - cached.ts < 86400000)) {
      faceIdDescriptors = cached.data || [];
      console.log('[FaceID] Cache hit:', faceIdDescriptors.length, 'descriptors');
      // Skip straight to camera — same path as successful network load
      hideFaceIdLoading();
    ...
    console.warn('[FaceID] Cache read failed, using network:', e);
  }
  ...
  try {
    const { data: students, error } = await sb
      .from('Database')
      .select('nama, kelas, ekstra, face_id')
      .not('face_id', 'is', null);

    if (error) throw new Error(error.message || "Gagal memuat data wajah");

    faceIdDescriptors = [];
    (students || []).forEach(s => {
      const descriptor = parseFaceVector(s.face_id);
    ...
    console.log('[FaceID] Network load:', faceIdDescriptors.length, 'descriptors');

    // === SAFE CACHE SAVE ===
    safeCacheSet(safeCacheKey('faceid_desc', currentSemester), { ts: Date.now(), data: faceIdDescriptors });
